...
def load_hs_ply(
    ...
    # Draw hotspot_prob
    if (
        "vertex_hsprob" in mesh.get_attribute_names()
        and "vertex_nx" in mesh.get_attribute_names()
    ):
        hotspot_prob = mesh.get_attribute("vertex_hsprob")
        color_array_surf = charge_color(hotspot_prob)
        for tri in faces:
            vert1 = verts[int(tri[0])]
            vert2 = verts[int(tri[1])]
            vert3 = verts[int(tri[2])]
    ...
    # Draw hotspot_prob
    if (
        "vertex_hspred" in mesh.get_attribute_names()
        and "vertex_nx" in mesh.get_attribute_names()
    ):
        hotspot_pred = mesh.get_attribute("vertex_hspred")
        color_array_surf = charge_color(hotspot_pred)
        for tri in faces:
            vert1 = verts[int(tri[0])]
            vert2 = verts[int(tri[1])]
            vert3 = verts[int(tri[2])]
...
def load_hs_giface(filename, color="white", name="giface", dotSize=0.2, lineSize=1.0):
    mesh = pymesh.load_mesh(filename)
